...
- **Image preloading** - All quiz images (question images and image options) are preloaded immediately after registration. Uses `new Image()` to cache images in browser before quiz starts. Preloading happens both for new registrations and returning users (session verification). Images from options starting with `/` are detected as image paths. Ensures instant image display during quiz.
