...
- **Decimal points** - `points` may be a decimal with at most 2 places (e.g. `0.5`, `1.25`); must be > 0. Validation applies to all question types (it used to run only for text questions). Server normalizes every points value and sum through `normalize_points()`/`sum_points()` in `server.py`, which round to 2 decimals and return an int when whole, so sums like 0.1+0.2 report as 0.3 and not 0.30000000000000004. Templates mirror this with `roundPoints()`/`formatPoints()` JS helpers; the admin editor's points input uses `step="0.01" min="0.01"` and drops any value that is not a positive number (so a typed `0` saves as the default 1 instead of a value the server would reject).
