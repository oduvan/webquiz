...
def test_list_files_csv():
    with custom_webquiz_server() as (proc, port):
        ...
        data = response.json()
        assert "csv" in data
def test_list_files_invalid_type():
    with custom_webquiz_server() as (proc, port):
        response = requests.get(
...
def test_files_view_nonexistent_quiz():
    with custom_webquiz_server() as (proc, port):
        response = requests.get(
...
def test_admin_list_quizzes():
    with custom_webquiz_server() as (proc, port):
        ...
        assert "quizzes" in data
        assert isinstance(data["quizzes"], list)
def test_admin_switch_quiz_success():
    with custom_webquiz_server() as (proc, port):
        # First, list quizzes
        list_response = requests.get(
            f"http://localhost:{port}/api/admin/list-quizzes", headers={"X-Master-Key": "test123"}
...
def test_admin_switch_quiz_nonexistent():
    with custom_webquiz_server() as (proc, port):
        response = requests.post(
            f"http://localhost:{port}/api/admin/switch-quiz",
...
def test_admin_get_quiz_content():
    with custom_webquiz_server() as (proc, port):
        # First, list quizzes
        list_response = requests.get(
            f"http://localhost:{port}/api/admin/list-quizzes", headers={"X-Master-Key": "test123"}
...
def test_submit_answer_without_registration():
    with custom_webquiz_server() as (proc, port):
        response = requests.post(
        ...
        data = response.json()
        assert "error" in data
def test_submit_answer_invalid_question_id():
    with custom_webquiz_server() as (proc, port):
        # First register
...
def test_verify_user_valid_cookie():
    with custom_webquiz_server() as (proc, port):
        # Register user
        reg_response = requests.post(f"http://localhost:{port}/api/register", json={"username": "TestUser"})
        user_id = reg_response.json()["user_id"]
