...
def test_registration_with_single_field():
    ...
    with custom_webquiz_server(config=config) as (proc, port):
        ...
        response = requests.post(f'{base_url}/api/register',
                               json={'username': 'student1', 'grade': '10'})
        assert response.status_code == 200
        data = response.json()
        assert 'user_id' in data
...
def test_registration_with_multiple_fields():
    ...
    with custom_webquiz_server(config=config) as (proc, port):
        ...
        response = requests.post(f'{base_url}/api/register',
                               json={'username': 'student2', 'grade': '11', 'school': 'Central HS'})
        assert response.status_code == 200
        data = response.json()
        assert 'user_id' in data
...
def test_registration_field_validation():
    ...
    with custom_webquiz_server(config=config) as (proc, port):
        ...
        # Missing grade field
        response = requests.post(f'{base_url}/api/register',
                               json={'username': 'student3', 'school': 'Test School'})
        assert response.status_code == 400
        assert 'error' in response.json()

        # Missing school field
        response = requests.post(f'{base_url}/api/register',
                               json={'username': 'student3', 'grade': '9'})
        assert response.status_code == 400
        assert 'error' in response.json()
...
def test_user_csv_creation(temp_dir):
    ...
    with custom_webquiz_server(config=config) as (proc, port):
        ...
        # Register a user
        response = requests.post(f'{base_url}/api/register',
                               json={'username': 'student4', 'grade': '12', 'school': 'North HS'})
        assert response.status_code == 200
...
def test_user_csv_content(temp_dir):
    ...
    with custom_webquiz_server(config=config) as (proc, port):
        ...
        # Register a user
        response = requests.post(f'{base_url}/api/register',
                               json={'username': 'student5', 'grade': '9'})
        assert response.status_code == 200
...
def test_answers_csv_uses_user_id(temp_dir):
    ...
    with custom_webquiz_server() as (proc, port):
        ...
        reg_response = requests.post(f'{base_url}/api/register',
                                    json={'username': 'answeruser'})
        assert reg_response.status_code == 200
        user_id = reg_response.json()['user_id']

        # Submit an answer
        answer_response = requests.post(f'{base_url}/api/submit-answer',
                                       json={
                                           'user_id': user_id,
                                           'question_id': 1,
                                           'selected_answer': 1
...
def test_csv_naming_convention(temp_dir):
    ...
    with custom_webquiz_server(config=config) as (proc, port):
        ...
        # Register user
        reg_response = requests.post(f'{base_url}/api/register',
                                    json={'username': 'pairtest', 'grade': '10'})
        assert reg_response.status_code == 200
        user_id = reg_response.json()['user_id']

        # Submit answer
        requests.post(f'{base_url}/api/submit-answer',
...
def test_registration_with_cyrillic_field_labels(temp_dir):
    ...
    with custom_webquiz_server(config=config) as (proc, port):
        ...
        response = requests.post(f'{base_url}/api/register',
                               json={'username': 'ukrainian_student', 'клас': '8', 'школа': 'Гімназія №5'})
        assert response.status_code == 200
        user_id = response.json()['user_id']
