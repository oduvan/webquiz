...
Вкладка **Config** надає два способи редагування конфігурації сервера:

**Режим Form** (за замовчуванням) — структурована форма для редагування налаштувань реєстрації:
- **Require admin approval** — прапорець для увімкнення/вимкнення схвалення адміністратором
- **Username label** — налаштування мітки поля імені користувача
- **Additional registration fields** — додавання/видалення додаткових полів (наприклад, Клас, Школа)

**Режим YAML** — текстовий редактор для прямого редагування YAML з повним доступом до всіх параметрів конфігурації.

Використовуйте перемикач **Form** / **YAML** для переключення між режимами. Якщо є незбережені зміни, з'явиться запит на підтвердження. Після збереження обидва режими автоматично оновлюються.
