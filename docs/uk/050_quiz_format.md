...
show_final_list: true        # false щоб приховати список питань на екрані результатів
...
> **Ручне управління:** Адміністратор може примусово відкрити відповіді, не чекаючи на відстаючих учнів, використовуючи кнопку "Відкрити Відповіді для Всіх" в адміністративній панелі. Докладніше див. розділ "Адміністративний інтерфейс".
...
#### `show_final_list` (необов'язковий)

Визначає, чи відображається таблиця з переліком питань на фінальному екрані результатів.

- **`true`** (за замовчуванням) — показує підсумок балів та таблицю з кожним питанням, відповіддю учня та правильністю
- **`false`** — показує лише підсумок балів (наприклад, "3/5 (60%)")

```
show_final_list: false  # Показувати лише бали, без списку питань
```

> **Порада:** Використовуйте `show_final_list: false`, коли хочете, щоб учні бачили лише загальний результат без перегляду окремих питань.
...
#### `stick_to_the_previous` (необов'язковий, для питань)

Визначає, чи повинне питання залишатися поруч із попереднім питанням при рандомізації. Це корисно для пов'язаних питань, які повинні йти послідовно (наприклад, текст для читання з питаннями до нього).

- **`true`** — питання завжди йде безпосередньо після попереднього (формує групу)
- **`false`** (за замовчуванням) — питання може бути переміщене незалежно

```yaml
questions:
  - question: "Прочитайте текст і дайте відповідь на питання 1-3"
    options:
      - "A"
      - "B"
      - "C"
    correct_answer: 0
  - question: "Питання 2 до тексту"
    options:
      - "A"
      - "B"
      - "C"
...
> **Як це працює:** Питання з `stick_to_the_previous: true` формують групи з попередніми питаннями. При рандомізації групи перемішуються як цілі блоки, зберігаючи внутрішній порядок. Наприклад, якщо Q1→Q2→Q3 утворюють групу, вони завжди будуть йти в цьому порядку, але група може опинитися в будь-якому місці тесту.

> ⚠️ **Важливо:** Перше питання не може мати `stick_to_the_previous: true` (немає попереднього питання, до якого прив'язуватися).

> **В адмін-панелі:** Питання з `stick_to_the_previous: true` позначаються індикатором 🔗 у згорнутому вигляді.
...
Бали також можуть бути дробовими числами — **не більше двох знаків після коми**:

```yaml
- question: "Питання на пів бала"
  options:
    - "A"
    - "B"
  correct_answer: 0
...
**Правила для `points`:**
- Має бути числом, більшим за 0
- Не більше двох знаків після коми. `1.234` буде відхилено при збереженні квізу
- Редактор квізів в адмін-панелі покаже помилку з номером питання, а не округлить значення сам
- За замовчуванням — `1`
...
- Питання, які оцінюються не в 1 бал, показують індикатор 🏆 під час тестування
...
- **points** — бали за правильну відповідь, число з не більш ніж 2 знаками після коми (за замовчуванням: 1)

**Код перевірки (checker):**
- Використовує змінну `user_answer` (відповідь студента)
- Доступний модуль `math` (використовуйте `math.sqrt`, `math.sin`, тощо)
...
✅ Захист від копіювання — учні не можуть виділяти або копіювати текст питань
