...
Окрім питань з вибором, WebQuiz підтримує питання з текстовою відповіддю, де студент вводить текст у поле. Питання з полями `checker` або `correct_value` автоматично визначаються як текстові:
...
- **question** — текст питання
- **checker** або **correct_value** — принаймні одне з них обов'язкове для визначення текстового питання
...
- **checker** — Python код для перевірки відповіді (необов'язково)
...
- Доступні функції: `sqrt`, `sin`, `cos`, `tan`, `log`, `exp`, `pi`, `e`, `abs`, `len`, `int`, `float`, `str`, `list`, `dict`, `range`, `sorted`, `sum`, `max`, `min`
- Якщо код викидає будь-яке виключення, відповідь вважається неправильною
- Якщо checker не вказано, використовується точне співпадіння з `correct_value`
